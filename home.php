									...
									<?php
										...
										while($row = $query->fetch_assoc()){
											...
											while($crow = $cquery->fetch_assoc()){
												...
												$input = ($row['max_vote'] > 1z) ? '<input type="checkbox" class="flat-red '.$slug.'" name="'.$slug."[]".'" value="'.$crow['id'].'" '.$checked.'>' : '<input type="radio" class="flat-red '.$slug.'" name="'.slugify($row['description']).'" value="'.$crow['id'].'" '.$checked.'>';
