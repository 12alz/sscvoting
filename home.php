                                        ...
                                        <?php
                                            ...
                                            while($row = $query->fetch_assoc()){
                                                ...
                                                echo '
                                                    <div class="row">
                                                        <div class="col-xs-12">
                                                            <div class="box box-solid" id="'.$row['id'].'" data-maxvote="'.$row['max_vote'].'">
...
$(function(){
    ...
    $('#ballotForm').submit(function(e){
        ...
        let isValid = true;
        $('form#ballotForm .box').each(function(){
            const maxVote = $(this).data('maxvote');  // Get the max_vote for this position
            const selectedCount = $(this).find('input:checked').length;  // Count selected candidates

            // Validate based on max_vote for each position
            if (selectedCount < maxVote) {
                isValid = false;
                alert('You need to select at least ' + maxVote + ' candidates for ' + $(this).find('.box-title').text());
                return false; // Stop checking further
            }

            if (maxVote == 1 && selectedCount > 1) {
                isValid = false;
                alert('You can only select one candidate for ' + $(this).find('.box-title').text());
                return false;
            }
        });
