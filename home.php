...
$(function(){
    ...
    // Limit selection to 2 candidates per position
    $('input[type="checkbox"].flat-red').on('change', function() {
        var $checkboxes = $(this).closest('ul').find('input[type="checkbox"].flat-red');
        if ($checkboxes.filter(':checked').length > 2) {
            $(this).prop('checked', false);
            $('.message').html('You can only select up to 2 candidates per position');
            $('#alert').show();
        }
    });
});
