                                ...
                                <form method="POST" id="ballotForm" action="submit_ballot.php">
                                    ...
                                    while($row = $query->fetch_assoc()){
                                        ...
                                        while($crow = $cquery->fetch_assoc()){
                                            ...
                                            // Set input type based on max_vote
                                        ...
                                        echo '
                                            <div class="row">
                                                <div class="col-xs-12">
                                                    <div class="box box-solid" id="'.$row['id'].'" data-max-votes="'.$row['max_vote'].'">
...
<script>
// JavaScript to manage voting logic

$(function(){
    // Initialize iCheck for checkboxes and radios
    $('.content').iCheck({
        checkboxClass: 'icheckbox_flat-green',
        radioClass: 'iradio_flat-green'
    });

    // Reset button click event to uncheck all selected options for a given position
    ...
    // Preview Button - check if any votes were made
    ...
    // Enable/Disable checkboxes based on max_votes
    $("input[type='checkbox']").on('ifChecked', function() {
        var positionId = $(this).attr('name');
        var maxVotesAllowed = $(this).closest('.box').data('max-votes');
        var selectedCount = $("input[name='" + positionId + "[]']:checked").length;

        if (selectedCount >= maxVotesAllowed) {
            $("input[name='" + positionId + "[]']").not(':checked').prop('disabled', true);
        }
    });
    ...
    $("input[type='checkbox']").on('ifUnchecked', function() {
        ...
        if (selectedCount < maxVotesAllowed) {
            $("input[name='" + positionId + "[]']").prop('disabled', false);
        }
    });
});
</script>
