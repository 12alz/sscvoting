                                ...
                                <form method="POST" id="ballotForm" action="submit_ballot.php">
                                    <?php
                                        ...
                                        while($row = $query->fetch_assoc()){
                                            ...
                                            // Instructions for max vote
                                            $instruct = ($row['max_vote'] > 1) ? 'You may select up to '.$row['max_vote'].' candidates' : 'Select only one candidate';

                                            echo '
                                                <div class="row">
                                                    <div class="col-xs-12">
                                                        <div class="box box-solid" id="'.$row['id'].'" data-maxvotes="'.$row['max_vote'].'">
...
$(function(){
    ...
    // Function to check the number of selected checkboxes for each position
    function updateCheckboxState() {
        $('.box').each(function() {
            var positionId = $(this).attr('id'); // Get position ID
            var maxVotes = parseInt($(this).data('maxvotes')); // Get max votes allowed for this position
            
            // Get the number of selected checkboxes for this position
            var checkedCount = $("input[name^='" + positionId + "']:checked").length;

            // If the maximum number of selections is reached, show a warning message
            if (checkedCount >= maxVotes) {
                $(".message").html("You can only select up to " + maxVotes + " candidates for this position.");
                $('#alert').show();
            } else {
                $('#alert').hide();
            }
