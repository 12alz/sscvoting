...
function sendResetEmail($email, $reset_code) {
    ...
    $mail->Body = "Use this OTP Code to reset your password: " . $reset_code . "<br/>" . 
                  "Click the link to reset password: http://mccsscvoting.com/admin/set-password.php?reset&email=$email";
...
if (isset($_POST["btn-forgotpass"])) {
    ...
    if ($result && $result->num_rows > 0) {
        ...
        $update_sql = "UPDATE `admin` SET `code` = '$reset_code' WHERE email = '$email'";
...
if (isset($_POST["btn-new-password"])) {
    ...
    if ($result && $result->num_rows > 0) {
        ...
        if ($get_code && $otp === $get_code) {
            ...
            $update_sql = "UPDATE `admin` SET `password` = '$hashed_password', `code` = '$reset' WHERE email = '$email'";
            ...
            header("Location: ../admin/set-password.php");
            exit();
        }
    } else {
        $_SESSION["notify"] = "No user found with this email. Please try again.";
