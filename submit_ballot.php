<?php
	...
	if(isset($_POST['vote'])){
		// Check if at least one candidate has been selected
		if(count($_POST) == 1){
			...
			while($row = $query->fetch_assoc()){
				...
				if(isset($_POST[$position])) {
					...
					// If multiple candidates can be selected
					if ($max_votes > 1) {
						// Check if the user has selected more than the allowed number of candidates
						...
						// If only one candidate can be selected
